  ...
  return (
    <section className="panel settlement-panel">
      ...
      <section className="settlement-detail-section">
              ...
              {result.detailRows.map((row) => {
                ...
                return (
                  <tr key={row.expenseId}>
                    ...
                    <td>{formatKrw(row.estimatedKrwAmount)}</td>
                    <td>
                    ...
                    {trip.members.map((member) => (
                      <td key={`${row.expenseId}-${member.id}`}>{formatKrw(row.memberDisplayShares[member.id] ?? 0)}</td>
                    ))}
                    <td>{formatKrw(row.memberDisplayShareTotal)}</td>
                    <td>{row.note}</td>
                  </tr>
                );
              })}
        ...
        {layoutMode === 'mobile' ? (
          <p className="hint-text">모바일에서는 표를 가로로 스크롤해 실제 금액 입력과 멤버별 부담금을 확인하세요.</p>
        ) : null}
      </section>
    </section>
  );
