...
async function patchInstalledCopilotSdk(projectRoot: string): Promise<void> {
  const targets = [
...
async function loadCopilotSdkModule(projectRoot: string): Promise<CopilotSdkModule> {
  ...
  return (await import(pathToFileURL(sdkPath).href)) as CopilotSdkModule;
}
...
function getProviderRank(model: ListedModel): number {
...
function getVersionScore(model: ListedModel): number {
  const text = `${model.id} ${model.name}`.toLowerCase();
  const regex = /\d+(?:\.\d+)?/g;
...
function chooseAutomaticModel(models: ListedModel[]): string {
...
function dataUrlToTempFileName(imageName: string, mimeType: string): string {
  const safeName = path
...
async function dataUrlToTempFile(image: OcrImageInput): Promise<string> {
...
function normalizeOcrPayload(parsed: Record<string, unknown> | null, rawText: string): OcrExtraction {
  if (!parsed) {
    return {
      amount: null,
      currency: null,
...
function normalizeCsvMapping(parsed: Record<string, unknown> | null, headers: string[]): CsvMapping {
...
class CopilotRuntime {
  private approveAll: unknown = null;
  private client: any = null;
  private initialized = false;
  private modelCache: ListedModel[] | null = null;
  ...
  private async invoke(prompt: string, attachmentPaths: string[] = []): Promise<{ model: string; text: string }> {
    await this.ensureClient();
    ...
    try {
      ...
      const messageOptions: Record<string, unknown> = { prompt };
  ...
  async suggestCsvMapping(headers: string[], sampleRows: string[][]): Promise<{ mapping: CsvMapping; model: string; rawText: string }> {
    const prompt = [
  ...
  async extractOcr(images: OcrImageInput[]): Promise<{ model: string; results: OcrExtraction[] }> {
    const results: OcrExtraction[] = [];
    let model = '';
...
let runtimeSingleton: CopilotRuntime | null = null;
